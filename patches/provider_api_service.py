...
from rag.llm import ChatModel, EmbeddingModel, ModelMeta, OcrModel, RerankModel, Seq2txtModel, TTSModel
...
def _make_speech2text_probe_wav() -> str:
...
async def _verify_speech2text_model(mdl, timeout_seconds: int):
    # Only OpenAI-compatible clients share a safe synchronous transcription
    # contract here. Other Seq2txt implementations may long-poll or inherit a
    # Base implementation that is not usable before provider-specific setup.
    if not (hasattr(mdl, "client") and hasattr(mdl.client, "audio")):
        return None

    path = _make_speech2text_probe_wav()
    try:
        text, token_count = await asyncio.wait_for(
            asyncio.to_thread(mdl.transcription, path),
...
async def verify_api_key(provider_name: str, api_key: str|dict, base_url: str=None, region: str=None, model_info: list[dict]=None):
    ...
    chat_passed, embd_passed, rerank_passed, ocr_passed, tts_passed, asr_passed = False, False, False, False, False, False
    ...
    for llm in factory_llms:
        ...
        if not embd_passed and LLMType.EMBEDDING.value in model_types:
            ...
            try:
                ...
                msg += f"\nFail to access model({provider_name}/{llm['llm_name']})." + str(e)
        elif not asr_passed and LLMType.SPEECH2TEXT.value in model_types:
            if provider_name not in Seq2txtModel:
                unsupported_msg = f"Speech2text model from {provider_name} is not supported yet."
                logging.warning(unsupported_msg)
                msg += f"\n{unsupported_msg}"
                continue
            try:
                mdl = Seq2txtModel[provider_name](key=api_key_str, model_name=llm["llm_name"], base_url=base_url)
                await _verify_speech2text_model(mdl, timeout_seconds)
                asr_passed = True
            except Exception as e:
                logging.exception(
                    "Fail to access speech2text model for provider=%s model=%s",
                    provider_name,
                    llm["llm_name"],
                )
                msg += f"\nFail to access model({provider_name}/{llm['llm_name']})." + str(e)
        if any([embd_passed, chat_passed, rerank_passed, ocr_passed, tts_passed, asr_passed]):
            msg = ""
            break

    success = any([embd_passed, chat_passed, rerank_passed, ocr_passed, tts_passed, asr_passed])
